...
def analyze_code_chunk(code_chunk, chunk_number=1, total_chunks=1):
...
def analyze_code_with_groq(code_snippet):
    ...
    # If code is small enough (considering input + output tokens), analyze directly
    # Groq limit is 6000 tokens total, so we keep input under 4000 to leave room for response
    if estimated_tokens <= 4000:
        return analyze_code_chunk(code_snippet)
    
    # For large files, split into chunks
    st.info(f"Large file detected ({estimated_tokens} estimated tokens). Analyzing in chunks...")
    chunks = split_code_into_chunks(code_snippet, max_tokens=3000)  # Smaller chunks for analysis
    ...
    for i, chunk in enumerate(chunks):
        ...
        chunk_analysis = analyze_code_chunk(chunk, i+1, len(chunks))
        if chunk_analysis and not chunk_analysis.startswith("Error"):
    ...
    summary_section = generate_analysis_summary(analysis_results)
    final_analysis = f"{summary_section}\n\n---\n\n{combined_analysis}"
                ...
                if content and not content.startswith("Error"):
                    with st.spinner("Analyzing code with AI..."):
                        try:
                            ...
                            # Provide download option for large analysis results
                            if len(analysis_result) > 2000:
                                st.download_button(
                                    label="📥 Download Analysis Report",
                                    data=analysis_result,
                                    file_name=f"analysis_{selected_file.replace('/', '_')}.md",
                                    mime="text/markdown"
                                )
                        except Exception as e:
